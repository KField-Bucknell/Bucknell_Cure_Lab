...
BIOL205_206 <- read_excel("Grade Outcomes/BIOL205_206 Grade Distribution_First Years by Gender & Ethnicity.xlsx", 
  ...
  select("Academic Period Description", "Course Identification", "Student Classification", 
         "Gender Description", "Ethnicity Description", "Final Grade") %>%
...
summary(model1_202)
model_full_202 <- glm.nb(DFW ~ Curriculum * Gender * `Race/Ethnicity`, data = Only202_DFW)
...
```{r, echo=FALSE}
newdata_202 <- data.frame(Curriculum = c("Old", "New", "Old", "New")) %>%
  mutate('Race/Ethnicity' = c("White", "White", "nonWhite", "nonWhite"))
predict.glm(selected_model_202, newdata=newdata_202, type="response")
selected_model_202
...
```{r, echo=FALSE}
cat_plot(selected_model_202, pred = Curriculum, modx = `Race/Ethnicity`, interval = TRUE)
```
...
catplot_202 <- cat_plot(selected_model_202, pred = `Race/Ethnicity`, modx = Curriculum, 
...
summary(model1_201)
model_full_201 <- glm.nb(DFW ~ Curriculum * Gender * `Race/Ethnicity`, data = Only201_DFW)
...
```{r, echo=FALSE}
newdata_201 <- data.frame(Curriculum = c("Old", "New", "Old", "New")) %>%
  mutate('Race/Ethnicity' = c("White", "White", "nonWhite", "nonWhite"))
predict.glm(selected_model_201, newdata=newdata_201, type="response")
selected_model_201
...
catplot_201 <- cat_plot(selected_model_201, pred = `Race/Ethnicity`, modx = Curriculum, 
         interval = TRUE, errorbar.width = 0.5, 
         pred.labels = c("White Students","Students of Color"),
         modx.labels = c("Prior", "CURE"),
         x.label = "", y.label = "DFW Likelihood")
